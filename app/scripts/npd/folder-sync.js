  ...
  , function ($scope, Database, googleApi, utils)
    {
      ...
      function syncFolder (fileId) {
        ...
        api.files.get({ fileId : fileId })
          .execute( function(meta){ 

            if (meta && !meta.error) {
              if (meta.mimeType == 'application/vnd.google-apps.folder' && meta.title.match(/[A-Z]+\d{1,2}\-\d{3,4}[A-Z]*/)) {
            ...
            if (resp && !resp.error) {
              ...
              if (resp.nextPageToken) {
                window.setTimeout(function () { 
                  syncChildren(angular.extend( query,{ pageToken : resp.nextPageToken }))
                  }, 10)
              } else {
                chkSyncing ()
              }
      ...
      function getFolders (query) {
        ...
        if (!query)
          query = { q: 'mimeType = "application/vnd.google-apps.folder" and trashed = false and hidden = false'}
              ...
              angular.forEach(resp.items, function (meta) {
                ...
                if (! meta.title.match(/[A-Z]+\d{2}\-\d{3,4}[A-Z]*/)) {
                  ...
                  var title = meta.title
                    , data  = { title: title, id : meta.id, modifiedDate: meta.modifiedDate }
                  ...
                  if (!meta.parents || meta.parents[0].isRoot) {
                    ...
                    api.files.get({fileId : meta.parents[0].id})
                ...
                else { console.log('skip', meta.title)}
              })

              if (resp.nextPageToken) {
                window.setTimeout(function () { 
                  getFolders(angular.extend (query, { pageToken : resp.nextPageToken }))
                  }, 10)
              }
      ...
      $scope.doSync = function () {
        if ($scope.imageFolder) {
          ...
          $scope.syncCount = 0
          $scope.syncFinish = 0
          syncChildren ({ folderId: $scope.imageFolder, q: 'trashed = false' })
        }
      }
