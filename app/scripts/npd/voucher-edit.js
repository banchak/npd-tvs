  ...
  , function ($scope, legacyEditDI, $controller, Database, GDrive, $filter, $routeParams)
    {
      ...
      function selected2Item() {
        ...
        if (adds.length) {
          ...
          angular.forEach (adds, function (taken){
            items.push({name : taken.name, price : taken.price, $temp : taken.$temp })
          })
        }        
      }
      ...
      function _verifyItemTaking (state, field) {
        ...
        return function (item) {
          ...
          if (data.state == state) {
            ...
            if (taking.person!=person || taking.date!=date || taking.price!=item.price || taking.voucher!=voucher)
      ...
      function _postVerify (unpost, verifyItem) {
        ...
        if ($scope.postState=='cancelled' || (!unpost && $scope.postState=='posted')) {
          ...
          errors.push({ error : 'already '+$scope.postState, name : $scope.resource._name })
        }
        ...
        if (!errors.length) {
          ...
          if ($scope.postState=='pending') {
            ...
            if (posted && (now - posted) < 30 ) {
              ...
              errors.push({ error : 'other is posting', name : $scope.resource._name })
            }
          }
        }
        ...
        if (!errors.length) {
          angular.forEach($scope.items(),function(item) {
            ...
            err = verifyItem(item)
            if (err)
              errors.push({ error : err, name : item.name })
          })
        }
      ...
      function _postTaking(state, field) {
        ...
        field = field || 'info.taking'
        angular.forEach($scope.items(),function(item) {
          var pm, _sys, taking, qry, changes
            , data = tempdata.get(item)
          ...
          if ((data.state||null) == state) {
            ...
            pm = productDb.dataAccess.bulkUpdate( qry, changes).then(function(resp){
              if (!resp) {
                ...
                errors.push({ error : 'database not response', name : item.name })
              }
              else if (resp.error) {
                
                errors.push({ error : resp.error, name : item.name })
              }
              else if (!resp.n) {

                errors.push({ error : 'data conflict', name : item.name })
              }
              return resp
            })
            promises.push(pm)
      ...
      function postTaking (unpost, state, field) {
        ...
        promise = promise.then(function() {
          ...
          return $scope.editOpr.postState(!unpost && 'posted',(!unpost && 'ผ่านรายการ') || 'คืนรายการ').then(function(data){
            if (data.error)
              return $q.reject()    
          })
        })
      ...
      function _postGetting(unpost) {
        ...
        angular.forEach($scope.items(),function(item) {
          var pm, _sys, meta, taking, kept, qry, changes
            , data = tempdata.get(item)
          ...
          if ((data.state||null)==state) {
            ...
            if (state) {
              ...
              changes = {$unset : {}, $set : { _sys : _sys, meta : meta, 'info.keeping' : keeping }}
              changes.$unset[field] = ''
            }
            else {
              ...
              changes = { $set : {_sys : _sys, meta : meta} }
              changes.$set[field] = taking

              if (kept)
                changes.$set['info.keeping'] = kept
              else
                changes.$unset = {'info.keeping' : ''}
          ...
          var services = {
            ...
            beforeSave : function (savedata) {
              ...
              if (savedata._name.match(/\*$/)) {
                var pattern, xsegs
                  , xx = ({'ยืม' : 'VT', 'คืน' : 'VG', 'ขาย' : 'VS' })[savedata._type] || '**'
            ...
            prohibitPrice : function (item, clear) {
              var flag
                , price = utils.lookup(item,'$temp.data.info.lowest_price')
