    ...
    .controller('productEditCtrl', ['$scope', 'legacyEditDI', '$controller', 'Database', 'GDrive',
      ...
      function($scope, legacyEditDI, $controller, Database, GDrive, $routeParams) {
        ...
        var success = function() {
          ...
          var services = {
            ...
            beforeSave: function(savedata) {
              ...
              if (savedata._name.match(/\*$/)) {
                var pattern, xsegs, xx = ($scope.resource._type || '')[0],
                  yy = moment().format('YY'),
                  sep = '-',
                  digit = 4
          ...
          $scope.resource$entry.init('meta', ['gems', 'providings', 'repairings', 'tooks',
            'kepts', 'refs'
          ])
          $scope.resource$entry.init('info', ['buying', 'taking', 'keeping', 'selling', 'metal',
            'watch'
          ])
          ...
          function calc() {
            ...
            this.calc_provider_cost = function() {
              ...
              if (+($scope.buying().cost)) {
                self.provider_cost_percent = _percent(+($scope.buying().provider_cost), +(
                  $scope.buying().cost))
              }
            }
            ...
            this.calc_target_price_percent = function() {
              ...
              if (self.total_cost) {
                self.target_price_percent = _percent((+($scope.resource.info.target_price) -
            ...
            this.calc_lowest_price_percent = function() {
              ...
              if (self.total_cost) {
                self.lowest_price_percent = _percent((+($scope.resource.info.lowest_price) -
                  self.total_cost), self.total_cost)
              }
              self.total_cost
            }
          ...
          if ($routeParams.opr == 'getting') {
            ...
            if ($scope.getState() != 'taken') {
              uis.errorBox('Sorry,"' + $routeParams.opr + '" is not allow!').open().then(
                function() {
                  $scope.editOpr.exit()
                })
            }
