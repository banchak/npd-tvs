...
.factory(
  'Database', ['$mongolabResourceHttp', '$q', 'SCHEMA', 'BUILT_IN', 'COLLECTIONS', 'utils',
    function($mongolabResourceHttp, $q, SCHEMA, BUILT_IN, COLLECTIONS, utils) {
      ...
      database.legacy.prototype.resource = function(schema) {
        if (!schema) {
      ...
      database.legacy.prototype.rawScopeQuery = function(keyword, scopeFields) {
        var self = this,
          kmatch, xcmd
        ...
        xcmd = keyword.match(/^(\!)?\@([a-z._]+)\=(.+)$/)
        if (xcmd) {
          var q = {}, qdef = _qdef(xcmd[3], xcmd[1])

            if (angular.isArray(qdef)) {
              return qdef.length == 1 ? qdef[0] : {
                $or: qdef
              }
            }
