...
angular.module('modules.google-api',[])
  ...
  , function ($timeout, $q, $rootScope, $http, GAPI_CONFIG) {
    ...
    function _gapi () {
      ...
      function _wait () {
        ...
        if (!patience) {
          ...
          deferred.reject({ 
            error: { code : 408, message : 'gapi not instantiate.' } 
          })
          delete _gapi.promise
          _safe$apply()
          return
    ...
    this.auth    = this._gapi.then(function(api) { return api.auth})
    this.client  = this._gapi.then(function(api) { return api.client})
    ...
    this.client.load = function (apiname, version, callback) {
      ...
      function _chkReply() {
        ...
        if (!reply) {
          ...
          deferred.reject({ 
            error: { code : 408, message : 'client.' + apiname + '.' + version + ' not load.' } 
          })
    ...
    this.authorize = function (req) {
      ...
      if ($rootScope.authorizeData) {
        ...
        , function () {
            ...
            $rootScope.authorizeData = { user : false }
            deferred.resolve($rootScope.authorizeData)
            delete self.authorize.promise
            self.safe$apply()
        })
        ...
        apiquery.execute( function (resp) {
          ...
          if (resp && resp.error && resp.error.code == 401) {
            ...
            self.authorize(req || {}).then( 
              function () { apiquery.execute(resolve) }
              , resolve)
            return
          }
          resolve(resp)
