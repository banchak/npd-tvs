...
.service('GCalendar', ['$q', 'utils', 'googleApi', 'GAPI_CONFIG', function( $q, utils, googleApi, GAPI_CONFIG){
  ...
  this.newCalendar = function (resource) {
    ...
    return this.useCache(fileId, function () {
      ...
      pm.then(function () { self.useCache('calendars', false) })
  ...
  this.events = function (query, max) {
    ...
    return this.calendars().then(function (resp) {
      ...
      if (promises.length) {
        return $q.all(promises).then(function() { return items })
      }
    })
