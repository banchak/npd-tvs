...
angular.module('modules.gdrive', ['modules.google-api', 'modules.utils'])
  .service('GDrive', ['$q', 'utils', 'googleApi', 'GAPI_CONFIG',
    function($q, utils, googleApi, GAPI_CONFIG) {
      ...
      this.cabinets = function(cabname, appname) {
        ...
        if (!appname.then) {
          // creat a promise
          appname = this.useCache(appname, function() {
            return self.folders(appname)
          })
        }
            ...
            if (cabs) {
              ...
              angular.forEach(cabs, function(cab) {
                angular.forEach(cab.items, function(item) {
                  ...
                  cablist.push({
                    owner: cab.app.ownerNames[0],
                    iconLink: cab.app.iconLink,
                    id: item.id
                  })
                })
              })
